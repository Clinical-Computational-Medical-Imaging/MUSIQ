...
def test_real_eq1_volume_no_longer_bypasses_the_imagetype_check(collector, tmp_path, irregular_spacing_ct_series_dir):
    """Regression test for first finding `_select_ct_volume` used to `return eq[0]` before
    `_rank()` -- the only PRIMARY/SECONDARY ImageType check -- ever ran, so *any* Eq_1 candidate
    won regardless of its
    ImageType. The slice-spacing test above already uses this same real series (it is dcm2niix's
    own trigger for an `_Eq_1` output) but only checks the resulting physical extent, not which
    file was picked or why -- it would not notice if the ImageType check were skipped. This test
    makes that mechanism itself the assertion, using real dcm2niix output rather than a synthetic
    directory listing:
    ...
    2. dcm2niix writes a JSON sidecar (with a valid ``PRIMARY`` ImageType) only for the plain
       volume, never for the ``_Eq_1`` one -- so the plain volume's PRIMARY status is confirmed,
       while the Eq_1 file's ImageType is undiscoverable (no sidecar at all, i.e. strictly *less*
       information than "confirmed SECONDARY" would be).
    3. `_select_ct_volume` now picks the plain, confirmed-PRIMARY volume over the
       ImageType-undiscoverable ``_Eq_1`` one -- the fixed ranking treats "no sidecar" the same
       as "not primary" (see `test_missing_sidecar_is_treated_as_non_primary` in
       `test/unit/series_selection/test_select_ct_volume.py`), so it no longer takes the Eq_1
       shortcut ahead of the PRIMARY/SECONDARY check.

    `repair_slice_spacing_from_dicom` derives its correction directly from the DICOM
    `ImagePositionPatient` extent (not from whichever volume was chosen), so the slice-spacing
    test above stays correct regardless of which of the two candidates ends up copied to
    `CT.nii.gz` here.
    """
