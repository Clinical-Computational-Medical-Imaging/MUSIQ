...
from musiq.series_selection import calculate_suv_factor, convert_pet
...
class TotalSegmentatorMuscleFatSUL:
        ...
        for top_dir in top_dirs:
            ...
            for dirpath, _, filenames in os.walk(top_dir_path):
                for filename in filenames:
                    ...
                    weight = data["Studies"][study_date]["Modalities"][modality][series_index][series_name]["DICOM"][
                        "PatientWeight"
                    ]
                    fat_in_percent = data["Studies"][study_date]["Modalities"][modality][series_index][series_name][
                        "body_composition_analysis"
                    ]["glut_to_c6"]["total_fat_in_%"]
    ...
    def convert_pet2sul(self, output_dirpath: str | os.PathLike, lean_body_mass: float, study_date: str) -> os.PathLike:
        ...
        if os.path.isfile(out_sul_fpath):
            ...
            with open(os.path.join(os.path.dirname(output_dirpath), "patient_info.json")) as f:
                data = json.load(f)

            series_name = next(iter(data["Studies"][study_date]["Modalities"]["PT"][0]))
            total_dose = data["Studies"][study_date]["Modalities"]["PT"][0][series_name]["DICOM"][
            ...
            start_time = data["Studies"][study_date]["Modalities"]["PT"][0][series_name]["DICOM"][
