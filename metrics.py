...
def calculate_patient_level_mtv_psmatv(
    ...
    if rel_thres:
        threshold = np.max(prod) * threshold
...
def calculate_suvpeak_median(ptarray, maskarray, spacing):
    ...
    Returns:
        float: The SUVpeak value.
    """
    prod = np.multiply(ptarray, maskarray)
    # Calculate the number of voxels along each axis to approximate 1 cm³
    kernel_size = []
    for sp in spacing:
        num_voxels = max(1, int(round(10.0 / sp)))  # 10 mm = 1 cm
        # Ensure the kernel size is odd to have a central voxel
        if num_voxels % 2 == 0:
            num_voxels += 1
        kernel_size.append(num_voxels)
    kx, ky, kz = kernel_size
    half_kx, half_ky, half_kz = kx // 2, ky // 2, kz // 2

    # Find the index of the voxel with the maximum SUV
    max_idx = np.unravel_index(np.argmax(prod), prod.shape)
    x, y, z = max_idx

    # Define the bounds of the region, ensuring they are within the array dimensions
    x_start = max(x - half_kx, 0)
    x_end = min(x + half_kx + 1, prod.shape[0])
    y_start = max(y - half_ky, 0)
    y_end = min(y + half_ky + 1, prod.shape[1])
    z_start = max(z - half_kz, 0)
    z_end = min(z + half_kz + 1, prod.shape[2])
    ...
    suvpeak = float(np.median(region))
    return suvpeak
