...
Because stages key off file existence, they **skip work when the output file already exists** (idempotent re-runs). Stages that need an upstream artifact (e.g. AutoPET needs `SUL.nii.gz`, which is produced by the `sul` stage from the `muscle_fat` stage's `PatientLBM`) will warn/skip if it is missing — task ordering matters.

### PET metrics: SUV vs SUL

Most quantification runs for one or both of `SUV` and `SUL` (`--pet-metric`, default both). The metric name drives file names and JSON keys (e.g. `PETseg.nii.gz`/`PETsegPath` for SUV vs `PETsegSUL.nii.gz`/`PETsegSULPath` for SUL). `SUL.nii.gz` (lean-body-mass-corrected SUV) is generated by the `sul` stage (`sul_computation.py`), which reads `PatientLBM` produced by the `muscle_fat` stage (`totalsegmentator_muscle_fat.py`); `muscle_fat` itself produces the tissue segmentation, body-composition stats and LBM.
